...
const (
	// deployment中的APP标签名
	APP_NAME = "guestbook-demo2-operator-app"
	// tomcat容器的端口号
	//CONTAINER_PORT = 8080
	// 单个POD的CPU资源申请
	CPU_REQUEST = "100m"
...
//+kubebuilder:rbac:groups=webapp.demo2.com,resources=guestbooks,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=webapp.demo2.com,resources=guestbooks/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=webapp.demo2.com,resources=guestbooks/finalizers,verbs=update
...
func (r *GuestbookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger.WithName("GuestBook-operator")
	// 实例化数据结构
	...
	logger.Info("3. instance : " + instance.String())
...
// 根据单个QPS和总QPS计算pod数量
func getExpectReplicas(elasticWeb *webappv1.Guestbook) int32 {
	// 单个pod的QPS
...
// 新建deployment
func createDeployment(ctx context.Context, r *GuestbookReconciler, elasticWeb *webappv1.Guestbook) error {
	//logger := r.L.WithValues("func", "createDeployment")
	logger := log.FromContext(ctx)
	logger.WithName("GuestBook-operator")
	logger.WithValues("func", "createDeployment")
							...
							Ports: []corev1.ContainerPort{
								{
									...
									ContainerPort: *elasticWeb.Spec.Port,
								},
							},
...
// 新建service
func createServiceIfNotExists(ctx context.Context, r *GuestbookReconciler, elasticWeb *webappv1.Guestbook, req ctrl.Request) error {
	...
	service = &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			Ports: []corev1.ServicePort{{
				...
				NodePort: *elasticWeb.Spec.PushPort,
			},
			},
	...
	// xxx 这一步非常关键！
	// 建立关联后，删除elasticweb资源时就会将svc也删除掉
	logger.Info("set reference")
...
// 完成了pod的处理后，更新最新状态
func updateStatus(ctx context.Context, r *GuestbookReconciler, elasticWeb *webappv1.Guestbook) error {
