...
func (c *Cpu) GetInfo(wlog *log.Wraplog) (interface{}, ErrorCollect) {
	...
	if err != nil {
		errors.Set("cpuLogicalCount", err)
	} else {
		c.LogicalCount = cpuLogicalCount
	}

	cpuPhysicalCount, err := cpu.Counts(false)
	if err != nil {
		errors.Set("cpuPhysicalCount", err)
	} else {
		c.PhysicalCount = cpuPhysicalCount
	}

	if len(errors) > 0 {
		return c, ErrorCollect(errors)
	}
	return c, nil
