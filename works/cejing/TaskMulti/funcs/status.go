...
func mem() {
	...
	g.Logge.Infof("内存监控 RSS占用:%d Kb, VSS占用:%d Kb", meminfo.RSS>>10, meminfo.VMS>>10)
...
func RunStatusWheel() {
	...
	g.Logge.Infof("开始运行时间 %s,已经运行%3d天%2d小时%2d分钟%2d秒,内存地址: %p", time.Unix(g.StartRunTime, 0).Format(g.TimeLayoutChi),
